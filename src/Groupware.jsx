...
function HomeScreen({ currentUser, notices, messages, events, onNavigate, onLogout }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:"#f0f4f8", overflow:"auto" }}>
      {/* バナー画像（全体表示） + 紺色ナビゲーションバーを白地部分に重ねる */}
      <div style={{ position:"relative", width:"100%", height:200, backgroundImage:"url('/bn.JPG')", backgroundRepeat:"no-repeat", backgroundSize:"100% auto", backgroundPosition:"0 -120px", flexShrink:0 }}>
        {/* 紺色ナビゲーションバー（白地部分にオーバーレイ） */}
        <div style={{ position:"absolute", top:0, left:0, right:0, background:"linear-gradient(135deg,#0f172a,#1e3a5f)", padding:"13px 16px", display:"flex", alignItems:"center", gap:10, boxShadow:"0 2px 12px rgba(0,0,0,0.3)" }}>
          <span style={{ fontSize:20 }}>💬</span>
          <span style={{ fontWeight:900, fontSize:16, color:"white", flex:1, letterSpacing:1 }}>グループウェア</span>
          <span style={{ fontSize:12, color:"rgba(255,255,255,0.7)" }}>{currentUser.avatar} {currentUser.nickname}</span>
          <button onClick={onLogout} style={{ padding:"5px 10px", borderRadius:8, border:"none", background:"rgba(255,255,255,0.12)", color:"rgba(255,255,255,0.9)", cursor:"pointer", fontSize:11, fontWeight:700 }}>🏠 ホームに戻る</button>
        </div>
        {/* 規約ボタン */}
        <button onClick={()=>setShowKiyaku(true)} style={{ position:"absolute", top:60, right:16, background:"rgba(15,23,42,0.85)", border:"none", borderRadius:10, padding:"6px 10px", cursor:"pointer", display:"flex", alignItems:"center", gap:4, color:"white", fontSize:11, fontWeight:700 }}>📜 規約</button>
      </div>
